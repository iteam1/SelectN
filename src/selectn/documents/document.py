...
class CodeDocument(Document):
    ...
    def preprocess(self) -> str:
        ...
        # 1. Remove comments (for simplicity, focusing on syntax structure)
...
class AntlrGrammarDocument(CodeDocument):
    """Specialized document class for ANTLR grammar files."""
    
    def preprocess(self) -> str:
        """
        Preprocess ANTLR grammar content for feature extraction.
        
        Returns:
            Preprocessed grammar content.
        """
        # For ANTLR grammars, we want to focus on rule structures
        lines = self.content.split('\n')
        processed_lines = []
        ...
        for line in lines:
            ...
            # Handle rule definitions specially
...
class DocumentFactory:
    ...
    @staticmethod
    def create_document(content: str, file_path: Optional[str] = None, 
        ...
        # Otherwise infer from file extension
            ...
            if ext in ['.g4']:
                doc_type = 'antlr'
            elif ext in ['.py', '.java', '.cpp', '.c', '.h', '.js', '.ts', '.go', '.rs', '.php', '.rb']:
                doc_type = 'code'
        ...
        if doc_type == 'antlr':
            return AntlrGrammarDocument(content, metadata)
...
class DocumentCollection:
    ...
    def add_from_file(self, file_path: str, doc_type: Optional[str] = None,
    ...
    def add_from_directory(self, directory_path: str, extensions: Optional[List[str]] = None,
                          recursive: bool = True, doc_type: Optional[str] = None) -> None:
        """
